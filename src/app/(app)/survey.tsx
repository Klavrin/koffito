...
import { ProfileFields, type ProfileFieldValues, validateAge } from "@/components/profile/profile-fields";
...
export default function SurveyPage() {
  ...
  const ageError = validateAge(details.age);
  const canContinue = step.kind === "about" ? !ageError : (answers[step.question.key]?.length ?? 0) > 0;
  ...
  return (
    <Screen
      header={<Header title={retake ? "Retake survey" : "Let's get to know you"} onBack={showBack ? handleBack : undefined} />}
      footer={
        <Button
      ...
      {step.kind === "about" ? (
        <SurveyStep key="about" section="Profile" question="Tell us more about you" hint="Everything here is optional — share what you like.">
          <View className="items-center gap-2">
            <Text variant="heading">Hi, {profile.firstName || "there"}! 👋</Text>
        ...
        <SurveyStep
          ...
          <OptionList
            ...
            onChange={(selected) => setAnswers((current) => ({ ...current, [step.question.key]: selected }))}
          />
        </SurveyStep>
      )}
    </Screen>
