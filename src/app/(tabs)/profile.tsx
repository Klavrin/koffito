...
export default function ProfilePage() {
  ...
  const coffeeTalks = events.filter(
    (event) => event.joined && event.status === "completed",
  ).length;
  const stats = [
  ...
  const personality = findSurveyOption(
    "personality",
    profile.survey.personality?.[0] ?? "",
  );
    ...
    >
      <View className="items-center gap-3">
        ...
        <View className="flex-row flex-wrap justify-center gap-2">
          {personality && (
            <Badge
              label={`${personality.emoji} ${personality.label.split(" (")[0]}`}
              variant="primary"
            />
          )}
          {meetup && (
            <Badge label={`${meetup.emoji} ${meetup.label.split(" (")[0]}`} />
          )}
        </View>
      ...
      <Section title="About me">
        <Card className="gap-3">
          <InfoRow
            icon="cafe-outline"
            label={`Favorite coffee: ${profile.favoriteCoffee ?? "still deciding"}`}
          />
          <InfoRow
            icon="person-outline"
            label={`Gender: ${profile.gender ?? "not shared"}`}
          />
          <InfoRow
            icon="calendar-outline"
            label={`Age: ${profile.age ?? "not shared"}`}
          />
          <InfoRow
            icon="briefcase-outline"
            label={`Occupation: ${profile.occupation ?? "not shared"}`}
          />
        </Card>
      </Section>
